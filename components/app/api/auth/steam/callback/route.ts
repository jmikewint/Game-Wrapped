...
export async function GET(request: NextRequest) {
  ...
  // Don't let a slow or failed library sync block sign-in — the user can
  // always hit /api/library/sync again from a "Refresh" action later.
  try {
    await syncOwnedGames(user.id, steamId);
  } catch (error) {
    console.error("Steam library sync failed during callback:", error);
  }
